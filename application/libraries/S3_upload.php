...
class S3_upload {
	...
	function upload_file($file_path)
	{
		...
		$s3_file = enkrip($file['filename']).'-'.rand(1000,1).'.'.$file['extension'];
		...
		if ($saved) {
			return array('uri'=>$this->s3_url.$this->bucket_name.'/'.$this->folder_name.$s3_file,'key'=>$this->folder_name.$s3_file);
		}
	}
	...
	function delete_file($uri)
	{
		...
		$deleted = $this->CI->s3->deleteObject(
			$this->bucket_name,$uri
		);
		if ($deleted) {
			return true;
		}
	...
	function lihat_file($uri)
	{
		// generate unique filename
		
		$berkas = $this->CI->s3->getObject(
			$this->bucket_name,$uri
		);
		return $berkas;
	}
